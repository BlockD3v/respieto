...
func _ready() -> void:
	set_initial_time()
	recalculate_time()   # emit initial values
...
func set_initial_time() -> void:
	...
	current_day = initial_day          # <-- set current_day
	current_minute = initial_minute    # optional, to avoid first tick

func recalculate_time() -> void:
	var total_minutes: int = int(time / GAME_MINUTE_DURATION)
	var day_minute: int = total_minutes % MINUTES_PER_DAY
	var hour: int = int(day_minute / MINUTES_PER_HOUR)
	var minute: int = int(day_minute % MINUTES_PER_HOUR)
	var day: int = int(total_minutes / MINUTES_PER_DAY)   # <-- calculate day
