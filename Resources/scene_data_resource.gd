...
func _load_data(window: Window) -> void:
	# If scene_file_path is empty, we can't instantiate – skip
	if scene_file_path == "":
		push_warning("SceneDataResource: scene_file_path is empty for node '", node_name, "'. Skipping load.")
		return
	...
	if node_path != null:
		...
		if scene_file_resource == null:
			push_error("Failed to load scene file: ", scene_file_path)
			return
